...
const Painel = () => {
  ...
  const [virandoPagina, setVirandoPagina] = useState(false);
  ...
  const verificarCamposPreenchidos = (estado) => {
    return [1, 2, 3, 4, 5].every(i => {
      ...
      return value && value.length === (i === 7 && estado === 'federal' ? 3 : 4);
    });
  };
  ...
  const moverParaProximo = (e, estado, rowIndex, maxLength) => {
    if (e.target.value.length >= maxLength) {
      ...
      // Lógica específica para o federal
      if (estado === 'federal') {
        if (rowIndex === 5) {
          // Após preencher o 5º prêmio, pular para o 4º
          const campo4 = e.target.closest("tbody").querySelector(`input[id="${estado}_milhar4"]`);
          if (campo4) campo4.focus();
        } else if (rowIndex === 4) {
          // Após preencher o 4º prêmio, pular para o 3º
          const campo3 = e.target.closest("tbody").querySelector(`input[id="${estado}_milhar3"]`);
          if (campo3) campo3.focus();
        } else if (rowIndex === 3) {
          // Após preencher o 3º prêmio, pular para o 2º
          const campo2 = e.target.closest("tbody").querySelector(`input[id="${estado}_milhar2"]`);
          if (campo2) campo2.focus();
        } else if (rowIndex === 2) {
          // Após preencher o 2º prêmio, pular para o 1º
          const campo1 = e.target.closest("tbody").querySelector(`input[id="${estado}_milhar1"]`);
          if (campo1) campo1.focus();
        } else if (rowIndex === 1) {
          // Após preencher o 1º prêmio, mostrar o container e pular para o 7º
          updateBlurContainerImage(estado, 1, e.target.value);
          const campo7 = e.target.closest("tbody").querySelector(`input[id="${estado}_milhar7"]`);
          if (campo7) campo7.focus();
        }
      } else {
        // Comportamento normal para Rio e Goiás
  ...
  const updateBlurContainerImage = (estado, rowIndex, value) => {
    ...
    } else if (estado === 'federal' && rowIndex === 1 && value.length === 4) {
      const dezenaBicho = value.substring(2);
      const grupo = getGrupo(dezenaBicho);
    ...
    } else if ((estado === 'rio' || estado === 'federal') && rowIndex === 1) {
      setBlurContainer({
        show: false,
        image: '',
        grupoNumero: '',
  ...
  // Atualização do handleInputChange
  const handleInputChange = (estado, rowIndex, value) => {
    let maxLength = 4;
    if (rowIndex === 7) {
      maxLength = estado === 'federal' ? 3 : 3;
    }
    ...
    setEstados(prev => {
      ...
      if (numericValue.length === maxLength) {
        ...
        // Efeito especial para o primeiro prêmio do Rio ou Federal
        if ((estado === 'rio' || estado === 'federal') && rowIndex === 1) {
          updateBlurContainerImage(estado, rowIndex, numericValue);
        }

        // Cálculo específico para o painel federal
        if (estado === 'federal' && rowIndex === 7 && numericValue.length === 3) {
          // Calcula o 6º prêmio (soma dos 5 primeiros)
          let soma = 0;
          for (let i = 1; i <= 5; i++) {
            const value = newState.federal[`milhar${i}`];
            if (!value || value.length !== 4) return newState;
            soma += parseInt(value);
          }
          ...
          newState.federal.milhar6 = sextoPremio;
          newState.federal.grupo6 = grupo6;
          newState.federal.nome6 = getNomeBicho(grupo6);
          newState.federal.img6 = grupo6 ? images.rg[grupo6.padStart(2, '0')] : '';
        }
      } else {
      ...
      // Cálculos normais para Rio e Goiás
      if (estado !== 'federal') {
  ...
  const updateResultadoInput = () => {
    ...
    setResultadoInput({
      ...
      federal: `${getResultado(federalTimes).name} ${dateStr}`
    });
  };
...
const alternarPainel = () => {
  ...
  const renderInputRow = (estado, rowIndex) => {
    ...
    if (estado === 'federal') {
      if (rowIndex === 6) {
        // 6º prêmio é calculado automaticamente quando o 7º é preenchido
        maxLength = 4;
        disabled = true;
      } else if (rowIndex === 7) {
        // 7º prêmio é manual no federal
        maxLength = 3;
        disabled = false;
      }
    } else {
      // Comportamento normal para Rio e Goiás
      maxLength = rowIndex === 7 ? 3 : 4;
      disabled = rowIndex === 6; // 6º prêmio é sempre desabilitado (calculado automaticamente)
    }
  ...
  return (
    <div className="centralizar-container">
      {/* Painel do Rio de Janeiro/Federal */}
      <div className="containder" 
           onMouseEnter={() => painelAtivo === 'rio' && setMostrarSeta(true)}
        ...
        {painelAtivo === 'rio' ? (
          ...
          <>
            <h1 className="title-jb">Resultados FEDERAL</h1>
            <input
              className="input-horaEData"
              maxLength="40"
              placeholder="Resultado Federal 00/00/0000 00:30h"
              type="text"
              id="federal_resultadoInput"
              value={resultadoInput.federal}
              disabled
            />
          </>
        )}
...
{painelAtivo === 'federal' && (
  <div 
    ...
    onClick={alternarPainel}
    title="Voltar para Rio"
  >
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M12 16L8 12L12 8" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
